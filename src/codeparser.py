...
class FunctionParser(ast.NodeVisitor):
    def __init__(self, node: ast.FunctionDef | ast.AsyncFunctionDef, class_name: Optional[str] = None):
        super().__init__()
        self.node = node
    ...
    def _parse_arguments(self) -> List[ArgumentInfo]:
        ...
        for i, arg in enumerate(default_args):
            default_value = None
            if self.class_name and i == 0 and arg.arg in ('self', 'cls'):
                continue  # 跳过 self 或 cls 参数
            if i >= offset: # 有默认值
    ...
    def _parse_methods(self) -> List[FunctionInfo]:
        ...
        for node in self.node.body:
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                func_parser = FunctionParser(node, self.node.name)
    ...
    def parse(self) -> ModuleInfo:
        ...
        for node in tree.body:
            if isinstance(node, ast.ClassDef):
                class_parser = ClassParser(node)
                class_infos.append(class_parser.parse())
            elif isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                func_parser = FunctionParser(node)
                function_infos.append(func_parser.parse())
    ...
    def _get_source_code(self) -> str:
        if self.source_code is None:
            ...
            try:
                ...
                print(f"[ERROR] Cannot decode file {self.file_path} with utf-8 encoding.")
                return None
            except Exception as e:
                print(f"[ERROR] An error occurred while reading file {self.file_path}: {e}")
                return None
        return self.source_code
    
    def __str__(self):
        return f"FileParser(file_path={self.file_path})"